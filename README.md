...
For complex tasks, it must use high specificity to ensure safety and success, which naturally lowers the ORI. The actual operational ceiling encountered is often the CR budget needed to pay for that specificity. A higher OAL is fundamentally linked to greater operational autonomy and capability within the framework, precisely because it enables the unit to execute more complex and resource-intensive (high CR cost) commands. **Low specificity high impact commands are stopped by a combination of OAL and TRF.**
